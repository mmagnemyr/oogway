...
if (-not $fileName) {
    Write-Host "Please provide the file name using -fileName parameter."
    Exit 1
}

# Specify the relative path to the Excel file
$relativePath = Join-Path -Path $PSScriptRoot -ChildPath "..\database"
Write-Host $relativePath
Write-Host $fileName
# Perform a case-insensitive search for the actual file name in the directory
$matchingFiles = Get-ChildItem -Path $relativePath -Filter *.xlsx | Where-Object { 
    $_.BaseName -ieq $fileName -or 
    ($_.BaseName -match '^([^_]+)_v\d+$' -and $matches[1] -ieq $fileName -and $_.Extension -eq '.xlsx') -or
    ($_ -match '^([^_]+)_v\d+\.xlsx$' -and $matches[1] -ieq $fileName) -or
    ($_ -eq $fileName)
}

# Check if a matching file is found
if ($matchingFiles.Count -eq 0) {
    Write-Host "No matching file found for the provided name."
    Exit 1
}

# If multiple matching files are found, choose the one without the "_vXXX" part
$actualFileName = $matchingFiles | Sort-Object { [int]($_.BaseName -replace '.*_v(\d+)$', '$1') } | Select-Object -Last 1

# Combine the relative path and the actual file name to get the complete file path
$filePath = Join-Path -Path $relativePath -ChildPath $actualFileName.Name

# Specify the path to the Excel executable
$excelPath = "C:\Program Files (x86)\Microsoft Office\root\Office16\EXCEL.EXE"  # Adjust this path accordingly
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUtMOgSe/BjV80DgXRcnb2pl/v
# aUGgghX/MIIC8jCCAdqgAwIBAgIQHlsuCbF3KYFDb2+p7pMvjTANBgkqhkiG9w0B
...
# BAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFGs6ExQCvzmr
# WjZcyOYpXtb93XlwMA0GCSqGSIb3DQEBAQUABIIBAJzdXe0VLUV9KUjIucMO5odv
# 5+4V8n2xi3enTQwZI5etw8kQNt6CRFndrzppGrQHlztIHZNdtqtp1Nh9YhwZrFOM
# mO45+AKwR6OaCc1ovJJtRoUiWpHU4ipdK1MvDQ9hGdDObCu0WxQwp6W+6mmBRhK3
# Mxjg1JCJtLXm+ow2NeT+frbtijlFmy26CTjsvIrQNbIOIS4YKF9yF8p0yDa2xm+O
# dw62EPoTHw3iej3PIP+2Pmqwo63kmXLQ4Ss0QEcAGwxmiTpwlity/QsASAo8GrDi
# MNCZD6CmPzNJ01h9j4H8VXFLtxpSlRjr32RMuc3Mmy6gQHScY94B7fT4cWlTIEeh
...
# ATAcBgkqhkiG9w0BCQUxDxcNMjQwMTI1MTAxMDI4WjAvBgkqhkiG9w0BCQQxIgQg
# VrluXyGJMVfJRgyVaYD9p03FVj3pKzHCGEz4AjWr3CswDQYJKoZIhvcNAQEBBQAE
# ggIAN23EKaQXigbNCCftTM+zGdckSl0xkVhUVdeCn8uc2jKQ96hZyLuFoyMGYeJ4
# nWjmTKFxUyan50ZfbqqLjatvSCY2Up+Y6c2taieVNUJRuMlbyI7ZwTatSpMtGXTT
# LY6x5VoKnZVo2xJZaXFt+PG6MVEfEozSVYGVj84ofqMDSsN383EolluJExnKaO1w
# NC1Xr+3EGj/xslbGBCLCaAsfT0reR2tynhLrgps113exOU9fzSSIQJXFeroHoHit
# xIlTwfWX7uQeVzQ7II7D3V5jfxdCQU1dtgiums7O3HUTMUF5r4upTQ1djCBRqVXP
# iyGRqWIxSmpzfeZZJ6ALQ1brtKKPb0cXq2XTZYyYO+jjw5n7HmPPi1XAZoheRbKL
# Ct23C77MicwyAy7NqTMaWVoHsb78PqP4pFvCrSqbr1OHBaZiz5wLg6g4zlL1z7EX
# LSIYM+T9zRWUQs7m+e9ZXEbUeoXs4E+BlOzVcaNxQlK4FoE/PRbQH3Y6dZ42fyRr
# vYHKohndLED8Dv6Tk+AMWzHkiw8DSnI3T/wYvHVnONvISuowlGf8S/+onGdMWsqJ
# cQVqLzMOVGEOpRS5r3g55Al/dGneHRgIi5BytgXe6iChZV8NlZ8gbEtI66qYfe6A
# MGGBxXI4Tqze5cbyCw6MIs0g1+Gc4NoPUBOKOoNMHk8pN+I=
